...
public class Data implements Ordenavel {
    ...
    public static final String[] DIAS_DA_SEMANA = { "Domingo", "Segunda-Feira", "Terça-Feira", "Quarta-Feira",
    ...
    public String diaDaSemana() {
        ...
        if (this.getMes() == 1 || this.getMes() == 2) {
            dia = (this.dia + (2 * (this.mes + 12)) + ((3 * ((this.mes + 12) + 1) / 5) + (this.ano - 1)
                    + ((this.ano - 1) / 4) - ((this.ano - 1) / 100) + ((this.ano - 1) / 400) + 1)) % 7;
        } else {
            dia = (this.dia + (2 * this.mes)
                    + ((3 * (this.mes + 1) / 5) + this.ano + (this.ano / 4) - (this.ano / 100) + (this.ano / 400) + 1))
                    % 7;
        }
    ...
    public boolean menorQue(Ordenavel o) {
        Data d1 = (Data) o;
        if (this.ano==d1.getAno()) {
            if (this.mes==d1.getMes()) {
                if (this.dia==d1.getDia()) {
                    return false;
                } else {
                    return this.getDia() < d1.getDia();
                }
            } else {
                return this.getMes() < d1.getMes();
            }
        } else {
            return this.getAno() < d1.getAno();
        }
    }
}